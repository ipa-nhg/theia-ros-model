...
@injectable()
export class KinematicsLanguageServerContribution extends BaseLanguageServerContribution {

    readonly id = KINEMATICS_LANGUAGE_SERVER_ID;
    readonly name = KINEMATICS_LANGUAGE_SERVER_NAME;

    getPort(): number | undefined {
        let arg = process.argv.filter(arg => arg.startsWith('--KINEMATICS_LSP='))[0];
        if (!arg) {
            return undefined;
        } else {
            return Number.parseInt(arg.substring('--KINEMATICS_LSP='.length), 10);
        }
    }
